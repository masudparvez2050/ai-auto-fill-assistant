...
function addAIButtonsToInputs() {
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.altKey && e.key === "a") {
      e.preventDefault();
      const container = activeElement.parentElement.querySelector(
        ".ai-autofill-container"
      );
      if (container) {
        const wandButton = container.querySelector(".ai-autofill-wand");
        if (wandButton) wandButton.click();
      }
    }
    ...
    if (e.altKey && e.key === "q") {
      ...
      if (container) {
        const askButton = container.querySelector(".ai-autofill-ask");
        if (askButton) askButton.click();
      }
    }

    // Alt+E for Enhance (only if content exists)
    if (e.altKey && e.key === "e") {
      e.preventDefault();
      const container = activeElement.parentElement.querySelector(
        ".ai-autofill-container"
      ...
      if (container) {
        const enhanceButton = container.querySelector(
          ".ai-autofill-enhance:not(.disabled)"
        );
        if (enhanceButton) enhanceButton.click();
      }
    }
  });
  ...
  inputElements.forEach((input, index) => {
    ...
    container.style.right = "10px"; // Debounce function for performance
        ...
        chrome.runtime.sendMessage(
          ...
          function (response) {
            ...
            if (chrome.runtime.lastError) {
              ...
              // Show more user-friendly error feedback
              const errorMessage = document.createElement("div");
              errorMessage.className = "ai-error-message";
              errorMessage.textContent =
                "AI service unavailable. Please check your API key or try again later.";

              // Position the error message
              errorMessage.style.position = "absolute";
              errorMessage.style.bottom = "-50px";
              errorMessage.style.left = "0";
              errorMessage.style.backgroundColor = "rgba(220, 53, 69, 0.9)";
              errorMessage.style.color = "white";
              errorMessage.style.padding = "6px 12px";
            ...
            chrome.runtime.sendMessage(
              ...
              function (response) {
                ...
                if (chrome.runtime.lastError) {
                  ...
                  errorElement.className = "ai-popup-error";
        ...
        chrome.runtime.sendMessage(
          ...
          function (response) {
            ...
            if (chrome.runtime.lastError) {
              ...
              errorMessage.className = "ai-error-message";
              errorMessage.textContent =
                "Content enhancement failed. Please check your API key or try again later.";
